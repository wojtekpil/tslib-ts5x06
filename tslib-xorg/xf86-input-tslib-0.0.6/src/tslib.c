...
static void ReadInput (InputInfoPtr local)
{
	...
	while((ret = ts_read(priv->ts, &samp, 1)) == 1) {
		...
		if(samp.pressure) {
			...
			x = samp.x;
			y = samp.y;
			ErrorF("In x= %d in Y= %d \n",samp.x,samp.y);
			//xf86XInputSetScreen(local->dev, priv->screen_num,
			//		samp.x,
			//		samp.y);
...
xf86TslibControlProc(DeviceIntPtr device, int what)
{
	...
	case DEVICE_INIT:
		...
		ErrorF("axiswidth: %d, axisheight: %d. \n",axiswidth,axisheight);
